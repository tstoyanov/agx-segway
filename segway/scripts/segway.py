...
def buildWheel(wheel_radius, wheel_height, wheel_type, wheel_pos):
    ...
    drive_rb.setRotation(agx.EulerAngles(0,0,pi/2))
...
class Segway():
  def __init__(self, **kwargs): 
    ...
    self.chassis = assembly.getRigidBody('base_frame')
    agxOSG.setDiffuseColor(agxOSG.createVisual(self.chassis, root()), agxRender.Color.LightGreen())

    self.left_wheel = assembly.getRigidBody('left_wheel')
    self.right_wheel = assembly.getRigidBody('right_wheel')
  ...
  def load_from_urdf() -> agxSDK.AssemblyRef:
    urdf_file = os.path.join(
        get_package_share_directory("segway"),
        "urdf",
        "segway_static.urdf")
    (package_path, tail) = os.path.split(get_package_share_directory("segway")) #getting rid of a dupliacate
